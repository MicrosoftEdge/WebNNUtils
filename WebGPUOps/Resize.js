...
const updateScales = (scales, axes, rank) => {
...
const validateInputs = (inputs, attributes, opsetVersion, scales, sizes, roi) => {
...
const getOriginalCoordinateFromResizedCoordinate = (coordinateTransferMode, dType) => `fn getOriginalCoordinateFromResizedCoordinate(xResized: u32, xScale: f32, lengthResized: u32,
    ...
    '}';
const getNearestPixelFromOriginal = (nearestMode, opsetVersion, dType) => `fn getNearestPixelFromOriginal(xOriginal: ${dType}, isDownSample: bool) -> ${dType} {` + (() => {
    switch (nearestMode) {
...
const initOutputShape = (inputShape, scales, sizes, axes) => {
    let outputShape = [];
    if (sizes.length > 0) {
        if (axes.length > 0) {
...
const adjustOutputShape = (inputShape, scales, attributes) => {
    const scaleInPolicy = (() => {
...
const calculateOriginalIndicesFromOutputIndices = (output, inputShape, outputShape, scalesLength, roiLength) => `
...
const calculateInputIndicesFromOutputIndices = (input, output, inputShape, outputShape, scalesLength, roiLength, useExtrapolation) => `
...
const checkInputIndices = (input, inputShape) => `
...
const setChannelAndBatchIndices = (input, channelIdx, batchIdx, spacialDims) => input.rank > spacialDims ? `
    ${input.indicesSet('input_indices', channelIdx, 'channel')};
    ${input.indicesSet('input_indices', batchIdx, 'batch')};
` :
    '';
const bilinearInterpolation = (input, output, inputShape, useExtrapolation, extrapolationValue) => {
    const isNchw = true;
...
const bicubicInterpolation = (input, output, inputShape, outputShape, scales, roi, cubicCoeffA, useExtrapolation, extrapolationValue, excludeOutside) => {
    ...
    const createCubicInterpolationFunction = (idx) => {
      ...
      fn ${direction}CubicInterpolation(input_indices: ${input.type.indices}, output_indices: ${output.type.indices}) -> ${dType} {
        ...
        var input_indices_copy: ${input.type.indices} = input_indices;
          ...
          data[i + 1] = ${idx === heightIdx ? input.getByIndices('input_indices_copy') :
...
const trilinearInterpolation = (input, output, inputShape, useExtrapolation, extrapolationValue) => {
    const isNchw = true;
...
const createResizeProgramInfo = (inputTensor, attributes, opsetVersion, scalesInput, sizes, roiInput) => {
    ...
    const getShaderSource = (shaderHelper) => `
      ...
      ${(() => {
        switch (attributes.mode) {
            case 'nearest':
              ...
              ${calculateInputIndicesFromOutputIndices(input, output, inputShape, outputShape, scales.length, roi.length, useExtrapolation)};
              `;
            case 'linear':
                return `
            ...
            ${(() => {
                    if (inputShape.length === 2 || inputShape.length === 4) {
                        return `${bicubicInterpolation(input, output, inputShape, outputShape, scales, roi, attributes.cubicCoeffA, useExtrapolation, attributes.extrapolationValue, attributes.excludeOutside)}`;
                    }
                    else {
      ...
      ${shaderHelper.mainStart()}
        ...
        switch (attributes.mode) {
            ...
            case 'linear':
                return `output[global_idx] = ${(inputShape.length === 2 || inputShape.length === 4) ? 'bilinearInterpolation' :
